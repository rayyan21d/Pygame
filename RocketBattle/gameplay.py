...
RED_SPACE_SHIP = pygame.transform.scale(
    RED_SPACE_SHIP_IMG, (SPACESHIP_WIDTH, SPACESHIP_HEIGHT))
...
def draw_window(red, yellow, red_bullets, yellow_bullets, red_health, yellow_health):
    ...
    # instead of loading them at some points we load
    WIN.blit(YELLOW_SPACE_SHIP, (yellow.x, yellow.y))
    # them wherever the rect is defined to contain our herso
...
def main():
    ...
    while run:
        ...
        for event in pygame.event.get():  # First thing to do
            ...
            if event.type == pygame.KEYDOWN:  # Thw other way of handling keypresses
                ...
                if event.key == pygame.K_RCTRL and len(red_bullets) < MAX_BULLETS:
                    bullet = pygame.Rect(
                        red.x+2, red.y+red.height/2 - 2, 10, 5)
        ...
        # gets all keys being pressed down atm 1 way of handling keys
        keys_pressed = pygame.key.get_pressed()

        # hah, a function for everything
        ...
        draw_window(red, yellow, red_bullets, yellow_bullets,
                    red_health, yellow_health)  # loop call to the drawing
